...
```
我们可以看到，已经有一个组件随着编辑器创建而加入了，即`minecraft:persistent`。`minecraft:persistent`组件用于控制生物是否是**宿存的** （**Persistent** ， _**持久化保存**_ ）。宿存的生物不会因距离过远而被引擎销毁。事实上，我们是不需要这个组件的。因为这不是类似于画之类的必须宿存的实体，因而其存在只会徒增引擎负担，所以我们将其删去。
...
![](/dev/mcmanual/mc-dev/assets/img/12.5_behavior_in-game.e8e3702d.gif)
...
![](/dev/mcmanual/mc-dev/assets/img/12.5_one_to_one.6afb3fb9.png)
...
![](/dev/mcmanual/mc-dev/assets/img/12.5_armor_in-game.fb7c65fd.png)
