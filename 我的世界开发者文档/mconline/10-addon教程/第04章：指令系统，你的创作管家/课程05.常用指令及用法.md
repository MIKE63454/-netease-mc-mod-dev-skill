...
![](/dev/mcmanual/mc-dev/assets/img/5_1.c448b6b6.png)

![](/dev/mcmanual/mc-dev/assets/img/5_2.98168491.png)

![](/dev/mcmanual/mc-dev/assets/img/5_3.0efa798f.png)
...
```
![](/dev/mcmanual/mc-dev/assets/img/5_4.afb29cce.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_5.7f85b209.png)

![](/dev/mcmanual/mc-dev/assets/img/5_6.e638daea.png)
...
在第一种格式中，[amount:int](<amount:int>)是必填参数，使用者输入正值增加经验值，输入负值减少经验值。经验值必须是整数且在0 ~ 2147483647之间，这是一个很庞大的范围，一般情况下，输入可接受的数值即可。[player: Target]是可省略参数，在不填写的情况下，指令生效在使用者本身。
...
![](/dev/mcmanual/mc-dev/assets/img/5_7.255de27c.png)

![](/dev/mcmanual/mc-dev/assets/img/5_8.46b4ab6a.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_9.8dec9dcc.png)

![](/dev/mcmanual/mc-dev/assets/img/5_10.d5fc48a0.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_11.a67e2e7e.png)

![](/dev/mcmanual/mc-dev/assets/img/5_12.aa1a1a9a.png)

![](/dev/mcmanual/mc-dev/assets/img/5_13.f74058cd.png)

![](/dev/mcmanual/mc-dev/assets/img/5_14.f231e0da.png)

![](/dev/mcmanual/mc-dev/assets/img/5_15.3aa9c5ae.png)

![](/dev/mcmanual/mc-dev/assets/img/5_16.997d7ff5.png)
...
```python
    ...
    /testfor Steve
```
![](/dev/mcmanual/mc-dev/assets/img/5_17.b4529a92.png)

![](/dev/mcmanual/mc-dev/assets/img/5_18.264f0ea5.png)
...
```
计分板系统是通过操作复杂的指令来追踪玩家分数的一种机制，根据玩家当前所带有的某种分数作为条件，可以用目标选择器来筛选出这一类玩家并执行相应指令。 第一个格式中，主要介绍如何使用scoreboard指令给实体目标添加分数类型。其中，[objective:string](<objective:string>) 给定一个分数名称。< displayName>给定一个显示名称，前者是必填参数，后者是选填参数。

第二个格式中，主要介绍如何使用scoreboard指令显示分数类型。

第三种格式中，主要介绍如何使用scoreboard指令移除分数类型。

第四种格式中，主要介绍如何使用scoreboard指令显示计分板，其中[list:sidebar](<list:sidebar>) 是必填参数，显示在list【列表】时，计分板内容会出现在暂停界面右侧的玩家列表里，显示在sidebar【侧边栏】时，内容会以界面地方式呈现在玩家屏幕右侧。[objective: string] 是选填参数，当不填写这一项时，会清空已经显示的列表或侧边栏的内容，但不会影响本来的分数值。当填写这一项时，会在列表或侧边栏上显示这一数值类型的值。[ascending:descending]是选填参数，由于值必定是整数类型，因此可以进行上升或者下降排序。
...
第七种格式中，主要介绍如何使用scoreboard指令将玩家的分数进行重置。其中[player:Target](<player:Target>) 是必填参数，需要让指令知道要将哪类玩家的分数进行重置，[objective:string]是选填参数，代指分数类型。重置意味着将分数值进行清空，即不再带有某个分数类型的值。

第八种格式中，主要介绍如何使用scoreboard指令将玩家的分数进行设置、加、移除。其中[player:Target](<player:Target>) 是必填参数，[objective:string](<objective:string>)是分数类型，[count:int](<count:int>)是值。set代指设置，即将玩家的分数设置成某个整数，而不进行任何数学计算，add代指添加，当玩家没有分数时，效果等同于“设置”，当玩家有分数时，效果等同于相加。remove是移除，当玩家没有分数时，分数会变成0 - 给定值，因此等同于设置值的负方向的值。当玩家有分数时，效果等同于相减。

第九种格式中，主要介绍如何使用scoreboard指令将玩家的分数进行随机区间的取值。其中[player:Target](<player:Target>) 是玩家选择器，属于必填参数，[objective:string](<objective:string>)是分数类型，属于必填参数，[min:int](<min:int>) [max:int](<max:int>)分别代指最小值和最大值。

第十种格式中，主要介绍如何使用scoreboard指令将一位玩家的分数进行计算，并最终将目标玩家分数设置为计算结果。[targetName:Target](<targetName:Target>) 是目标玩家选择器，最终改变分数的即是这个参数指定的那一类玩家。[targetObjective:string](<targetObjective:string>) 是目标玩家的分数类型。[operator:operator](<operator:operator>) 是运算符，一共包含%=[求模]、*=[相乘]、+=【相加】、-=【相减】、/=【相除】、<【若目标分数小于他人分数，则将目标分数设置为与他人分数相等】、=【将目标分数设置为与他人分数相等】、>【若目标分数大于他人分数，则将目标分数设置为与他人分数相等】、><【将目标分数与他人分数相互替换】。[selector:Target](<selector:Target>)可以代指其他玩家选择器，同样，这个他人可以指向自己，即自己的分数与自己做计算。[objective:string](<objective:string>)是分数类型。

![](/dev/mcmanual/mc-dev/assets/img/5_19.e289c1f3.png)

![](/dev/mcmanual/mc-dev/assets/img/5_20.bd874d9f.png)

![](/dev/mcmanual/mc-dev/assets/img/5_21.d21deab1.png)

![](/dev/mcmanual/mc-dev/assets/img/5_22.9ceb4a21.png)

![](/dev/mcmanual/mc-dev/assets/img/5_23.0d9239e3.png)

![](/dev/mcmanual/mc-dev/assets/img/5_24.bb5bc70e.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_25.1df2090f.png)

![](/dev/mcmanual/mc-dev/assets/img/5_26.7e5cf422.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_27.ef55a6fc.png)

![](/dev/mcmanual/mc-dev/assets/img/5_28.e9caa055.png)

![](/dev/mcmanual/mc-dev/assets/img/5_29.38038b93.png)
...
第二个格式中，[effect:Effect](<effect:Effect>) 是状态效果，是必填参数，[seconds:int] 是选填参数，即状态效果的持续时间，[amplifier:int] 是状态效果的强度，也是选填参数，[hideParticles:Boolean]是布尔参数，即是否隐藏状态效果粒子，true为隐藏，false为不隐藏，默认为false。

![](/dev/mcmanual/mc-dev/assets/img/5_30.9efa21b1.png)

![](/dev/mcmanual/mc-dev/assets/img/5_31.f9ee3825.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_32.edcec191.png)

![](/dev/mcmanual/mc-dev/assets/img/5_33.f5b26f10.png)
...
```
第一个格式中，[amount:int](<amount:int>)代指时间长度，即添加多少时间到当前时间中。
...
![](/dev/mcmanual/mc-dev/assets/img/5_34.4cfdb9de.png)

![](/dev/mcmanual/mc-dev/assets/img/5_35.b9cf5123.png)

![](/dev/mcmanual/mc-dev/assets/img/5_36.89b32026.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_37.b4337216.png)

![](/dev/mcmanual/mc-dev/assets/img/5_38.20e119f8.png)

![](/dev/mcmanual/mc-dev/assets/img/5_39.64ac8124.png)

![](/dev/mcmanual/mc-dev/assets/img/5_40.388c7b0e.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_41.222b72e3.png)

![](/dev/mcmanual/mc-dev/assets/img/5_42.72e2ba8c.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_43.51dc8102.png)

![](/dev/mcmanual/mc-dev/assets/img/5_44.0d3fa3d4.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_45.b0268042.png)

![](/dev/mcmanual/mc-dev/assets/img/5_46.57ad982f.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_47.d411c7df.png)

![](/dev/mcmanual/mc-dev/assets/img/5_48.2fac328f.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_49.5a96a5a9.png)

![](/dev/mcmanual/mc-dev/assets/img/5_50.fadeb22a.png)

![](/dev/mcmanual/mc-dev/assets/img/5_51.22d404a7.png)
...
```
第一个格式与第二个格式的区别在于，我的世界的有些游戏规则是只有是和否的选择，有些游戏规则则可以设置值规则。如是否生成怪物必定只有是和否的选项，出生范围则必定是可以设置值的规则。更多规则的概念，请参考[游戏规则 (opens new window)](<https://zh.minecraft.wiki/w/%E6%B8%B8%E6%88%8F%E8%A7%84%E5%88%99>)

![](/dev/mcmanual/mc-dev/assets/img/5_52.8a2eea30.png)

![](/dev/mcmanual/mc-dev/assets/img/5_53.ab76c67e.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_54.0ab0502a.png)

![](/dev/mcmanual/mc-dev/assets/img/5_55.87e03c61.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_56.65bb70af.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_57.017b29ec.png)

![](/dev/mcmanual/mc-dev/assets/img/5_58.3c4fc45c.png)
...
```
以消息“你好”为例，通常情况下使用其他聊天指令发给他人，或者使用聊天窗口聊天时，前面都会带有玩家名称。若使用者想要自行定制一个广播格式，则tellraw是非常好用的指令帮手。 /tellraw @a {"rawtext":[{"text":"[广播站]你好"}]}将会给所有玩家发送一个以广播站为前缀，内容为你好的消息。

![](/dev/mcmanual/mc-dev/assets/img/5_59.6b2bd12b.png)

![](/dev/mcmanual/mc-dev/assets/img/5_60.f6a4041b.png)
...
```
第一个格式中，[player:target](<player:target>)为玩家目标选择器，为必填参数，clear为清空玩家当前正在播放的标题文本。reset为重置标题的时间配置。

第二个格式中，[player:target](<player:target>)为玩家目标选择器，为必填参数，<title|subtitle|actionbar> 为屏幕大标题、屏幕副标题、快捷栏上方标题三种类型，为必填参数，titleText接收纯文本或者json消息，为必填参数。

第三个格式中，<fadeIn: int> 为渐进时间，<stay: int>为停留时间，<fadeOut: int>为淡出时间。该格式可以用来设置标题的时间配置。

![](/dev/mcmanual/mc-dev/assets/img/5_61.2c780944.png)

![](/dev/mcmanual/mc-dev/assets/img/5_62.4c1ae969.png)

![](/dev/mcmanual/mc-dev/assets/img/5_63.ed7d3f02.png)

![](/dev/mcmanual/mc-dev/assets/img/5_64.c9669a62.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_65.89a9fbcc.png)

![](/dev/mcmanual/mc-dev/assets/img/5_66.423639d3.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_67.ce37708b.png)

![](/dev/mcmanual/mc-dev/assets/img/5_68.285ff53a.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_69.8634bfe8.png)

![](/dev/mcmanual/mc-dev/assets/img/5_70.18c52089.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_71.8c63dde8.png)

![](/dev/mcmanual/mc-dev/assets/img/5_72.c810e794.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_73.32f806dc.png)

![](/dev/mcmanual/mc-dev/assets/img/5_74.52a96733.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_75.5f5a2fe4.png)

![](/dev/mcmanual/mc-dev/assets/img/5_76.0f6fa8f7.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_77.d790fc03.png)

![](/dev/mcmanual/mc-dev/assets/img/5_78.3a4ad51c.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_79.6fde29ca.png)
...
![](/dev/mcmanual/mc-dev/assets/img/5_80.5b031ca7.png)

![](/dev/mcmanual/mc-dev/assets/img/5_81.759f0b79.png)

https://mc.res.netease.com/pc/zt/20201109161633/mc-dev/assets/img/5_3.0efa798f.png
