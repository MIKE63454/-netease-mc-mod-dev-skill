...
arrayType | int | 渲染控制器arrays类型([渲染控制器arrays类型枚举](</dev/mcmanual/mc-dev/mcdocs/1-ModAPI/枚举值/RenderControllerArrayType.html>))  
arrayName | str | 数组名称  
...
bool | 是否成功  
  * 备注
    ...
    * 当生物离开再出现在视野时，不会恢复oneActor类渲染接口的设置，需要开发者监听[AddEntityClientEvent](</dev/mcmanual/mc-dev/mcdocs/1-ModAPI/事件/世界.html#addentityclientevent>)再次设置entity类渲染接口进行恢复
    * 某些生物受初始动画影响，几何体会有初始偏移，会导致修改几何体后表现异常。若修改后的几何体没有当前动画需要的骨骼时，会触发报错。
  * 示例
```python
    import mod.client.extraClientApi as clientApi
