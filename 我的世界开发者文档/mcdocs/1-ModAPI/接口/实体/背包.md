...
参数名 | 数据类型 | 说明  
---|---|---  
slotPos | int | [ArmorSlotType枚举](</dev/mcmanual/mc-dev/mcdocs/1-ModAPI/枚举值/ArmorSlotType.html>)枚举  
  * 返回值
...
list(tuple(int,int)) | 盔甲的附魔  
  * 备注
    ...
    * 如果物品不存在，或者没有附魔值，返回None。如果存在返回tuple数组，每个tuple由附魔类型([EnchantType枚举](</dev/mcmanual/mc-dev/mcdocs/1-ModAPI/枚举值/EnchantType.html>))和附魔等级组成
  * 示例
```python
    import mod.server.extraServerApi as serverApi
...
---|---|---  
posType | int | [ItemPosType枚举](</dev/mcmanual/mc-dev/mcdocs/1-ModAPI/枚举值/ItemPosType.html>)  
itemDict | dict | 生物身上不同位置的[物品信息字典](<../../../../mcguide/20-玩法开发/10-基本概念/1-我的世界基础概念.html#物品信息字典#物品信息字典>)列表，如果传入None将清除当前位置的物品/装备  
slotPos | int | 容器槽位，如果ItemPosType为左右手可不传，如果ItemPosType为背包则对应背包槽位，如果ItemPosType为armor则对应装备位置，具体请看[ArmorSlotType枚举](</dev/mcmanual/mc-dev/mcdocs/1-ModAPI/枚举值/ArmorSlotType.html>)  
  * 返回值
