# 运营指令
  ...
  * 使用方法
[code]post url: http:masterip:masterport/baseurl
        post body:
           json格式，例如：
           {
              "serverId":3,
...
[code]curl -X POST http://masterip:masterport/baseurl -d 'postbody'
```
若通过"/online-num/query"指令获取game在线人数，则可以在master上发送如下curl命令：
[code]curl -X POST http://11.11.11.11:8503/online-num/query -d '{"server_type":"game"}'
```
若需要使用商城插件neteaseShop中查询订单运营指令（/check-single-order指令），则可以在master上发送如下curl命令：
[code]curl -X POST http://11.11.11.11:8503/check-single-order -d '{"orderid" : 1234,"uid" : 123456789}'
```
## 查询UID
...
查询当前在线玩家的uid
    ...
    * 示例:
[code] post url: http://111.222.333.444:1101/netease/get-online-uids
    post body:
    {
    }
    response:
...
查询最近请求登录玩家的uid以及请求登录时间
    ...
    * 示例:
[code] post url: http://111.222.333.444:1101/netease/get-lazy-uids
    post body:
    {
    }
    response:
...
entity | dict | 关键字version为当前配置文件中，全局同时在线人数限制的版本号，暂时无用；关键字old_online_limit为修改前，全局最高同时在线人数的限制，返回0代表修改前，没有限制最高同时在线  
    * 示例:
[code] post url: http://111.222.333.444:1101/netease/update-player-online-limit
    post body:
    {
       "online_limit":8000
    }
...
onlineNum | int | 在线人数  
    * 示例:
[code] post url: http://111.222.333.444:1101/online-num/query
    post body:
    {
       "serverType":"game"
    }
...
onlineNum | int | 在线人数  
    * 示例:
[code] post url: http://111.222.333.444:1101/online-num/query-by-server-id
     post body:
     {
        "serverId":1
     }
...
debugLevel | bool | true:则日志设置为debug log level；false：日志设置为info log level  
    ...
    * 示例:
[code] post url: http://111.222.333.444:1101/conf/set-log-debug-level
    post body:
    {
       "debugLevel":false
    }
...
debugLevel | bool | true:则日志设置为debug log level；false：日志设置为info log level  
    ...
    * 示例:
[code] post url: http://111.222.333.444:1101/conf/set-server-log-debug-level
    post body:
    {
       "serverId":1,
       "debugLevel":false
...
查询所有服务器状态
    ...
    * response entity参数 dict。key是服务器id，value是服务器状态。服务器状态有： 1:断开连接状态 2:已连接状态 3:关服状态 4:优雅关服状态 6, 滚动更新中间状态，即将转换到状态7 7 也是滚动更新中间状态，即将转换到状态1或2

    * 示例:
[code] post url: http://111.222.333.444:1101/query-all-server-status
    post body:
    {
    }
    response:
...
7 也是滚动更新中间状态，即将转换到状态1或2  
    * 示例:
[code] post url: http://111.222.333.444:1101/query-one-server-status
    post body:
    {
    	"serverId":1
    }
...
type | str | 禁言类型，可自定义，公屏禁言为`Commom`  
    ...
    * 示例:
[code] post url: http://111.222.333.444:1101/silent
    post body:
    {
       "uid":11111,
       "banTime":40,
...
type | str | 禁言类型，可自定义，公屏禁言为`Commom`  
    ...
    * 示例:
[code] post url: http://111.222.333.444:1101/unban-silent
    post body:
    {
       "uid":11111,
       "type":"Commom"
...
reason | str | 禁言原因  
    ...
    * 示例:
[code] post url: http://111.222.333.444:1101/global-silent
    post body:
    {
       "isSilent":true,
       "reason":"系统维护"
...
reason | str | 踢出原因  
    ...
    * 示例:
[code] post url: http://111.222.333.444:1101/kickout-user
    post body:
    {
       "uid":11111,
       "reason":"玩家作弊",
...
bCombineReason | bool | 是否组合显示封禁原因。若为True，则按备注说明处理，否则被封禁玩家登陆会提示【reason】  
    ...
    * 示例:
[code] post url: http://111.222.333.444:1101/ban-user
    post body:
    {
       "uid":11111,
       "banTime":40,
...
uid | uint32 | 解除封禁玩家的uid  
    ...
    * 示例:
[code] post url: http://111.222.333.444:1101/unban-user
    post body:
    {
       "uid":11111,
    }
...
failUids | list(int) | 解除玩家在线标识失败的UID列表  
    * 示例:
[code] post url: http://111.222.333.444:1101/netease/release-online-lock
    post body:
    {
...
failUids | list(int) | 解除玩家在线标识的UID列表  
    * 示例:
[code] post url: http://111.222.333.444:1101/netease/release-online-lock-by-server
    post body:
    {
       "serverId": 4000,
    }
...
reason | str | 停服维护原因  
    ...
    * 示例:
[code] post url: http://111.222.333.444:1101/invalid-all-servers
    post body:
    {
       "invalid":true,
       "reason":"停服维护",
...
command | str | 服务器需要执行的控制台命令  
    * response entity参数 dict。key是服务器id，value也是一个dict，有两个key，code和message code=0代表在对应服务器上执行脚本成功，此时message中的信息为脚本中**print** 打印的信息； code=1代表在对应服务器上执行脚本失败，此时message中的信息为失败的原因；

    * 示例:
[code] post url: http://111.222.333.444:1101/netease/hunter-debug
    post body:
    {
...
command | str | 服务器需要执行的控制台命令  
    ...
    * 示例:
[code] post url: http://111.222.333.444:1101/hunter-debug
    post body:
    {
       "serverId":101,
       "script":"import time\nprint time.time()"
...
objNames | list | 通常是空  
    ...
    * 示例:
[code] post url: http://111.222.333.444:1101/check-memory-run
    post body:
    {
       "serverId":101,
...
bBegin | bool | true：开始profile；false：完成profile  
    ...
    * 示例:
[code] 性能数据文件名：profile_1574325974
    文件内容：
