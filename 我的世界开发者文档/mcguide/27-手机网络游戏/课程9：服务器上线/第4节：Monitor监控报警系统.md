...
**Monitor监控系统** 是基于第三方开源监控服务 **Grafana** 搭建的，详情及教程可以参考[**Grafana的官方网站**](<https://grafana.com>)，本教程只简要介绍如何在 **Apollo开服工具** 中使用 **Monitor监控系统** 。
...
### 配置监控数据库（**InfluxDB** ）访问参数
  ...
  * 在MCSTUDIO的**服务器配置** 中，在**更多** 分页下，下拉至**上线** 的标题下，勾选**监控运营数据** ，即可看到如下配置： ![choose variable value](/dev/mcmanual/mc-dev/assets/img/IMG20200604_160631.b4d0d0da.png)


**IP** 是influxdb的ip地址（上图的IP地址仅为示例） **数据库名称** 是你的项目独享的数据库名（上图的minecraft仅为示例） **账号** 是你的项目代号（上图的minecraft仅为示例） **密码** 是分配给你的influxdb的密码 **端口** 是influxdb的访问端口（上图的8086仅为示例）
...
### 使用运营数据统计插件
  ...
  * 使用官方插件中的运营数据统计插件（neteaseStatistics与neteaseStatisticsService），可以获得更多的运营相关数据，包括： 【daily_account_new】：新增账号数 【daily_account_login】：登录账号数 【daily_account_pay】：付费总值（钻石） 【daily_pay_rate】：付费率 【daily_pay_diamand】：人均付费值（钻石） 【daily_avg_oltime】：平均在线时长 【daily_avg_new_oltime】：新增用户平均在线时长 【daily_avg_login_cnt】：平均进入次数 【daily_retain1】：一天留存率 【daily_retain3】：三天留存率 【daily_retain7】：七天留存率
...
**域名** ：<https://monitor.apollo.netease.com>
...
使用给到的grafana账号密码登录 [**Monitor监控系统**](<https://monitor.apollo.netease.com>)

![login](/dev/mcmanual/mc-dev/assets/img/2019-05-15-20-46-23.84a3914c.png)

登录成功后进入 **Home Dashboard** ，鼠标指向左下角用户图标，发现目前正处于 **Main org.** 的组织中，点击它切换到自己项目组织下。

![click switch org](/dev/mcmanual/mc-dev/assets/img/2019-05-15-21-07-01.56f3fca6.png)

可以发现除了 **Main org.** 外还有另一个组织 **mctest** ，其实就是跟你的项目代号名称一样的组织，这个就是对应你的游戏的组织，点击 **Switch to** 切换到自己的组织。

![switch to your own org](/dev/mcmanual/mc-dev/assets/img/2019-05-15-21-10-38.363d9c85.png)

成功切换后，再次来到 **Home Dashboard** 面板，官方已经自动设置好了数据源，以及创建了一些监控图表。点击左侧的**Dashboard** ，再点击**Manage** 就可以看到根据模板预创建的监控图表

![open dashboard](/dev/mcmanual/mc-dev/assets/img/grafana_01.8edfb877.png) ![view dashboard](/dev/mcmanual/mc-dev/assets/img/grafana_02.32100d54.png)

### 了解更多

有关Dashboard和Panel的更多概念和细节，可以参考B站视频[**Grafana入门系列(4)——Dashboard和Panel**](<https://www.bilibili.com/video/BV1w5411n7Le/?spm_id_from=333.788.recommend_more_video.-1>)
...
![click notification channels](/dev/mcmanual/mc-dev/assets/img/2019-05-16-20-04-20.06d5cd2d.png)

点击中间的 **Add channel** 按钮创建第一个报警通道。

![add channel](/dev/mcmanual/mc-dev/assets/img/2019-05-16-20-11-16.fea8bc5a.png)
...
在报警通道设置界面：
  ...
  8. Url：正式服的报警服务器：<http://alerter.apollo.netease.com/alert?project=sample>[ (opens new window)](<http://alerter.apollo.netease.com/alert?project=sample>) 其中 sample 请填你的项目代号
  9. Http Method：选择 POST


之后点 **Send Test** 按钮检查是否配置正确，如果没问题，就点 **Save** 按钮保存。

![alert channel setting](/dev/mcmanual/mc-dev/assets/img/2019-05-16-20-22-19.be802fe5.png)
...
![panel create alert](/dev/mcmanual/mc-dev/assets/img/2019-05-16-20-31-41.63bde901.png)
...
![alert setting](/dev/mcmanual/mc-dev/assets/img/2019-05-16-20-53-45.aef8427e.png)

以上设置完毕后，报警即开始生效。可以在 **Alerting** 管理界面管理定制好的各项报警规则，可以暂停或启动。

![alert rules manage](/dev/mcmanual/mc-dev/assets/img/2019-05-16-20-55-42.9d048cde.png)

### 了解更多

有关基于Grafana的自定义报警，想要进一步了解可以参考B站视频[**Grafana入门系列(12)——基于Grafana的报警**](<https://www.bilibili.com/video/BV11v411W7gu?spm_id_from=333.999.0.0>)

https://mc.res.netease.com/pc/zt/20201109161633/mc-dev/assets/img/2019-05-15-20-46-23.84a3914c.png
