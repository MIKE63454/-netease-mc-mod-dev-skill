...
```
我们需要在这个环节 返回一个`List<String>` 然后就可以进行补全了 如果只想作为提示不想强行约束需要标记 (uncheck = true)

## 注册简单命令

如果想要注册一个 `/day` 的指令如何快速注册呢？  
这里利用到了 [自唤醒](</dev/mcmanual/mc-dev/mcguide/28-电脑网络游戏/课程4：常见第三方库教程/31-TabooLib/1-基础技术/4-自唤醒.html>) 功能
```python
