...
_目前为止, 配置API只有YAML配置功能可用. 这也是大多数插件为什么配置文件是YAML文件的原因.  
在本文中, 我们也将使用YAML配置API._  
_现在的配置API的类均在`org.bukkit.configuration` 和 `org.bukkit.configuration.file` 包中._  
_但是这不代表你只能使用 YAML，如果你有无限的创意和实现方法，你甚至可以将表格作为配置文件以实现更现代化的配表方式._
...
![](/dev/mcmanual/mc-dev/assets/img/0_4.3058dcae.jpg)
...
![](/dev/mcmanual/mc-dev/assets/img/0_5.aee20d81.jpg)

_后续会详细介绍, 这里需要知道判断方法._
...
```
_`saveResource`方法的第一个参数是文件名, 第二个参数是是否覆盖, 设置成false可以达到saveDefaultConfig的效果._
