...
![](/dev/mcmanual/mc-dev/assets/img/0_2.004430f1.jpg)
...
但这还不够，我们还需要告诉Bukkit，插件的一些信息才能让他更好的识别. 创建`plugin.yml`文件. 打开plugin.yml文件并在其中输入如下信息：
```python
    name: HelloWorld
...
_注意: 主类的名称并不是固定的, 但是`plugin.yml`文件的名称是固定的._
...
![](/dev/mcmanual/mc-dev/assets/img/0_3.dbcfd987.png)

# BukkitAPI中的Logger

## Logger

_这里只是简要提及, 不详细介绍, 只需要知道有这件事即可._
