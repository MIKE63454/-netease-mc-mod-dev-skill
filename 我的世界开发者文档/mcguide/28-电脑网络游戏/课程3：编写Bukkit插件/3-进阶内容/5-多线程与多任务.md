...
一是使用`org.bukkit.Bukkit.getScheduler()`或`org.bukkit.Bukkit.getServer().getScheduler()`获取`BukkitScheduler`实例. 例子:
```python
...
这里就不作更多介绍了. 欲了解更多内容和用法可以参考[Javadoc (opens new window)](<https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/Future.html>) 以及询问搜索引擎.
