...
详细的物品定义以及物品使用详见示例mod [CustomRangedWeaponMod](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/13-模组SDK编程/60-Demo示例.html#CustomRangedWeaponMod>)。
...
```
_注意：use_duration的大小应该与序列帧帧数相匹配_

  3. 自定义序列帧

3.1 将物品的序列帧贴图放到`textures\items`中，如：

![avatar](/dev/mcmanual/mc-dev/assets/img/custom_ranged_weapon01.ccaec446.png)

3.2 在textures/item_texture.json中增加序列帧图片的定义
[code] "customitems:ranged_weapon_frame": {
         	"textures": [
