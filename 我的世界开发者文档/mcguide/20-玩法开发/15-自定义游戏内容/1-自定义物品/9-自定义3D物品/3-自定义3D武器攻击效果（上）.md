...
**本节课程的[Demo请戳这里下载 (opens new window)](<https://g79.gdl.netease.com/CustomWeaponAttackFP.zip>)：**

![动画](/dev/mcmanual/mc-dev/assets/img/attack0_0.185ec6b9.gif)

## 设计手臂模型并重新调整金剑建模

打开前一节[自定义3D物品的Demo (opens new window)](<https://g79.gdl.netease.com/CustomWeapon3D.zip>)，目前金剑的骨骼会按照下列排序：
```python
    -root（玩家根骨骼）
    --waist（玩家上半身骨骼）
    ---body（玩家身体骨骼）
    ...
    ------sword（金剑骨骼）
```
![image-20220612014504234](/dev/mcmanual/mc-dev/assets/img/attack0_0.37d00bd6.png)

**我们设法在玩家手持金剑时，会在第一人称下显示两个手臂紧握着金剑。则此时两只手臂是跟随着金剑模型，而不牵扯到玩家模型本身。这样的设计意图给予了一些新的启发与可预料到的挑战。**

![任务计划甘特图 \(2\)](/dev/mcmanual/mc-dev/assets/img/attack0_1.7fe01aa6.png)
...
![任务计划甘特图 \(2\)](/dev/mcmanual/mc-dev/assets/img/attack0_2.d3134478.png)

除此之外，我们并不希望让arm骨骼会出现在第三人称视角。 **这是因为当玩家手持物品时，玩家模型的手臂会自动消失。但切换回第三人称将重新出现。玩家模型本身的手臂会与金剑的新双臂同时出现，影响观感。**

![任务计划甘特图 \(2\)](/dev/mcmanual/mc-dev/assets/img/attack0_3.33f3809e.png)
...
```python
    ...
    ------item（金剑骨骼）
```
![image-20220612025314595](/dev/mcmanual/mc-dev/assets/img/attack0_4.f712eecd.png)
...
```python
    ...
    ------item（金剑骨骼-不放置体块）
```
![image-20220612025523344](/dev/mcmanual/mc-dev/assets/img/attack0_5.0abe0a89.png)

## 制作第一人称手持金剑攻击动画

切换至动画模式，新建用以表现第一人称攻击动画sword_attack。

![image-20220612094740384](/dev/mcmanual/mc-dev/assets/img/attack0_6.a7dc28f1.png)

选中sword_attack动画，同时勾选first_hold动画，在手持第一人称动画生效的同时，以此视角基础设计挥舞动画。

![image-20220612095303937](/dev/mcmanual/mc-dev/assets/img/attack0_7.fdcb1892.png)

这里设计了一个0.75秒的攻击动画，在游戏里将实际从右往左挥舞金剑。其他形式可自行创作。

![动画1](/dev/mcmanual/mc-dev/assets/img/attack0_1.6fd9428b.gif)
...
![image-20220612102522220](/dev/mcmanual/mc-dev/assets/img/attack0_9.9795973f.png)

## 完成第一人称手持金剑攻击效果

使用自定义Query节点接口，可以最大限度地减少组件冲突的影响。自定义Query接口可以在客户端线程上自定义新的Query查询函数，可以内置在动画或动画控制器中，作为条件改变动画和动画控制器的效果。

![任务计划甘特图 \(2\)](/dev/mcmanual/mc-dev/assets/img/attack0_10.48a09824.png)

在行为包内创建Python脚本目录，取名CustomSwordScripts，并创建服务端系统和客户端系统。有关模组SDK目录创建和系统注册的模块，可以查看[此官网教程链接 (opens new window)](<https://g.126.fm/00FchC4>)。
```python
...
![动画2](/dev/mcmanual/mc-dev/assets/img/attack0_2.93582577.gif)