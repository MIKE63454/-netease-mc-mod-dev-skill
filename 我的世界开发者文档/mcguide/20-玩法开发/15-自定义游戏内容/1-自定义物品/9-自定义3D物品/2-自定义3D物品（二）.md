...
**本节课程Demo[戳这里下载 (opens new window)](<https://g79.gdl.netease.com/CustomWeapon3D.zip>)**
...
![img](/dev/mcmanual/mc-dev/assets/img/0_0.73b22e7d.png)

我的世界基岩版中，第一人称和第三人称的摄像机存在空间角度区别。因此需要单独为物品模型设计第一人称的手持动画。（下图为第三人称手持正确，但在第一人称显示意外的图例）

![image-20220608133313000](/dev/mcmanual/mc-dev/assets/img/0_1.2037767e.png)
...
![image-20220608133346326](/dev/mcmanual/mc-dev/assets/img/0_2.49a87b09.png)

需要将动画设置为循环播放，并将动画时长长度设置为0。

在对应的第一人称手持缩放动画（以下简称"first_scale"）插入一帧缩放关键帧，任意填入关键帧的缩放参数，这里我们填写XYZ为1.1并保存。

![image-20220608133438507](/dev/mcmanual/mc-dev/assets/img/0_3.5e27555a.png)

在对应的第一人称手持位置角度动画（以下简称“first_hold”）插入一帧关键帧同时调整位置和旋转值，这里我们任意填写XYZ的位置和旋转值为3，并保存。

![image-20220608133501905](/dev/mcmanual/mc-dev/assets/img/0_4.da27d049.png)
...
![image-20220608133527259](/dev/mcmanual/mc-dev/assets/img/0_11.ce0cf1f8.png)

打开Animation（动画）列表，点击Open Editor（打开编辑器）。

![image-20220608133539709](/dev/mcmanual/mc-dev/assets/img/0_10.c194923a.png)

默认情况下，编辑器窗口会很小。此时点击右下角的伸缩区域进行拉伸。

![image-20220608133552800](/dev/mcmanual/mc-dev/assets/img/0_9.9efb42e4.png)

拉到足够看清楚的大小后，直接往下拉到最底，这里展示着附着物挂载的动画状态。

![image-20220608133606005](/dev/mcmanual/mc-dev/assets/img/0_8.1b8f5147.png)

打开“first_hold”的Details（动画细节），再点击Bone Animation（骨骼动画），就可以看到前面用Blockbench制作的动画的各项值，并且支持直接在编辑器内动态修改，最后直观的体现在游戏内。

![image-20220608133619036](/dev/mcmanual/mc-dev/assets/img/0_7.8f3d0449.png)

这里我们对"first_hold"的position设置值为X为-8，Y为5，Z为-6。再对rotation设置X为-45，Y为15，Z为-135。对"first_scale"的scale设置为0.35。

![image-20220608133639677](/dev/mcmanual/mc-dev/assets/img/0_6.aefe9019.png)

再按F3按键直到界面消失，就可以在游戏内看到基本合格的第一人称自定义3D物品手持效果了。

![image-20220608133650381](/dev/mcmanual/mc-dev/assets/img/0_5.87dcb018.png)
