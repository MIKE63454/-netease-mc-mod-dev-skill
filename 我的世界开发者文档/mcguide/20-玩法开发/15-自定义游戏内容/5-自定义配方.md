...
```
![熔炉烧炼](/dev/mcmanual/mc-dev/assets/img/furnace_cactus.6ed58e49.png)
...
锻造转换配方和锻造纹饰配方需要物品具有特定的tag，需要在物品行为包添加[minecraft:tags](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/1-自定义物品/1-自定义基础物品.html#minecraft-tags>)组件。
...
在过去，假如你想支持任意原木合成家具，你需要添加多个配方（log、log2、以及后续更新的原木）。还需要时刻关注游戏更新，因为每新增一种原木，开发者就需要额外增加一个配方。 好消息是，在1.19.50之后，可以使用tag代替原来的item字段，将标签用作配方输入，直接表示一类物品，使合成表配置更简单。
