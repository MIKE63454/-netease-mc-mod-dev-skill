...
传送门demo为[portalGateDemo](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/13-模组SDK编程/60-Demo示例.html#portalGateDemo>)，并结合自定义维度使用。
...
### Mod配置说明
  ...
  1. DEMO配置两个带有传送组件的[自定义方块](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/0-自定义方块概述.html#customBlockJump>)，一个目标设置为自定义维度，一个设置到主世界
...
```
     * 设置传送门激活物品信息（骨粉）
[code] self.useOnItemName = 'minecraft:dye'
...
![](/dev/mcmanual/mc-dev/assets/img/portal_zuobiao.1f3871d0.png)
[code] self.touchPos =[(3,1),(3,2)]
```
  4. 激活检测

当玩家使用物品时监听 **ServerItemUseOnEvent** 事件，检测是否为设置的物品，如果是的话就进行传送门搭建的检测，具体的匹配算法接口见[ DetectStructure接口 ](<../../../../mcdocs/1-ModAPI/接口/世界/地图.html#detectstructure>)

对位置(3,1)或者(3,2)使用骨粉可以激活传送门，如下图所示。

![](/dev/mcmanual/mc-dev/assets/img/portal_active.d07b1b12.png)
  5. Portal Forcer功能

通过监听事件[ DimensionChangeFinishServerEvent ](<../../../../mcdocs/1-ModAPI/事件/玩家.html#dimensionchangefinishserverevent>)完成引擎内类似Portal Forcer功能（进地狱门传到地狱之后的逻辑）。
...
![image-20220830151510241](/dev/mcmanual/mc-dev/assets/img/portal_aux_assert.78892158.png)
