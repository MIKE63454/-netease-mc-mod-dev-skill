...
## 2.参考资料
  ...
  1. [官方文档 (opens new window)](<https://learn.microsoft.com/en-us/minecraft/creator/reference/content/featuresreference/examples/featuresintroduction?view=minecraft-bedrock-stable#json-format>)对特征及特征规则的解释与示例
...
结构指使用[结构方块 (opens new window)](<https://zh.minecraft.wiki/w/%E7%BB%93%E6%9E%84%E6%96%B9%E5%9D%97>)保存的地图模板，结构的编辑和导出需要玩家处于创造模式下，并开启实验性玩法。
...
若不希望结构中的空气方块在放置时覆盖掉其他方块，可通过指令`/give @p structure_void`获取[结构空位 (opens new window)](<https://zh.minecraft.wiki/w/%E7%BB%93%E6%9E%84%E6%96%B9%E5%9D%97#.E7.BB.93.E6.9E.84.E7.A9.BA.E4.BD.8D>)，并放置在相应的空气方块上。

![](/dev/mcmanual/mc-dev/assets/img/3-1.db6fe081.png)
...
![](/dev/mcmanual/mc-dev/assets/img/3-1-1.88f439de.png)

若结构中包含火方块，通过NeteaseStructureFeature放置的火与[永恒之火 (opens new window)](<https://zh.minecraft.wiki/w/%E7%81%AB#.E6.B0.B8.E6.81.92.E4.B9.8B.E7.81.AB>)相似，不会熄灭、不会扩散，但依旧有燃烧伤害。

#### 3-1-3.原版测试指令

**仅供测试，仅在pc开发包上可以使用** ，如需要在python代码中放置结构，见[python事件及接口](<#python%E4%BA%8B%E4%BB%B6%E5%8F%8A%E6%8E%A5%E5%8F%A3>)
...
参数 | 类型 | 解释  
---|---|---  
placement_pass | str | 后处理阶段一共分为若干个步骤，依次按顺序进行，处理完一个步骤的feature_rules后再处理下一个步骤的。这些步骤按顺序分别为：  
first_pass  
...
[code] query.is_biome(variable.originx + 15, variable.originz + 15, 2 ,4)
```
    * 备注
...
结构特征生成的事件，详见[ MOD SDK文档 ](<../../../../mcdocs/1-ModAPI/事件/世界.html#placeneteasestructurefeatureevent>)。

  * PlaceStructure接口


手动在世界中放置结构，详见[ MOD SDK文档 ](<../../../../mcdocs/1-ModAPI/接口/世界/地图.html#placestructure>)。
...
![](/dev/mcmanual/mc-dev/assets/img/3-3.d3edeea4.png)
...
假设想要以50%的概率的概率生成，那么需要在[处理群系边缘的情况](<#%E5%A4%84%E7%90%86%E7%BE%A4%E7%B3%BB%E8%BE%B9%E7%BC%98%E7%9A%84%E6%83%85%E5%86%B5>)的基础上加上这样的条件：
```python
    左下特征：
...
对于[处理群系边缘的情况](<#%E5%A4%84%E7%90%86%E7%BE%A4%E7%B3%BB%E8%BE%B9%E7%BC%98%E7%9A%84%E6%83%85%E5%86%B5>)，示例采用的是第一种解决方法，即只要黄色羊毛所在群系符合，则会生成整个结构，不在乎周围其他羊毛所在的群系。
...
![](/dev/mcmanual/mc-dev/assets/img/3-4.f6cef19d.png)
...
![](/dev/mcmanual/mc-dev/assets/img/3-5.3c90b56e.png)
...
![](/dev/mcmanual/mc-dev/assets/img/3-6.88d071f5.png)
...
![](/dev/mcmanual/mc-dev/assets/img/3-7.8acc5e19.png)
