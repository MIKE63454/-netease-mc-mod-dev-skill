...
以demo [CustomCommandDemo](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/13-模组SDK编程/60-Demo示例.html#CustomCommandDemo>)为例。

  1. 在behavior中新建`netease_commands`目录。

![avatar](/dev/mcmanual/mc-dev/assets/img/custom_command1.3e8f18b1.png)

  2. 在目录下新建一个json，编写指令的定义。json文件命名推荐为`团队名_模组命名空间_指令名称`，例如`craftStudio_customCommandDemo_explode`，文件名应使用英文数字和下划线，不可含有中文和特殊字符。

![avatar](/dev/mcmanual/mc-dev/assets/img/custom_command2.ee59f778.png)
...
type | str | 参数类型，具体见[CustomCommandTriggerServerEvent](<../../../mcdocs/1-ModAPI/事件/世界.html#customcommandtriggerserverevent>)事件的备注  
default | any | 参数默认值，可填写任意json数据格式  
  * 例如配置为`{"name": "浮点数", "type": "float"}`时，输入时会显示

![avatar](/dev/mcmanual/mc-dev/assets/img/custom_command3.c91f1481.png)

  * default不必和type的类型符合，**含有default的参数必须排在参数列表的尾部** ，且在游戏中可以不传。  
例如配置为`{"name": "浮点数", "type": "float", "default": null}`时，输入时会显示

![avatar](/dev/mcmanual/mc-dev/assets/img/custom_command4.a3dab750.png)


## 使用

  * 开发者需要监听[CustomCommandTriggerServerEvent](<../../../mcdocs/1-ModAPI/事件/世界.html#customcommandtriggerserverevent>)，为自定义指令编写功能。
  * 使用[SetCommand](<../../../mcdocs/1-ModAPI/接口/世界/指令.html#setcommand>)接口可以触发自定义指令。
  * 使用[GlobalCommandServerEvent](<../../../mcdocs/1-ModAPI/事件/世界.html#globalcommandserverevent>)可以监听所有指令，包括自定义指令。


## 配置变体

写好自定义指令的一套参数后，可以使用args1、args2......args9来额外配置多套参数，加上原来的args，最多能配置10套参数，开发者可以通过[CustomCommandTriggerServerEvent](<../../../mcdocs/1-ModAPI/事件/世界.html#customcommandtriggerserverevent>)的variant字段确定玩家发送的是哪一套参数。
```python
    {
        "format_version": "0.0.1",
        "name": "customtest3",
...
![avatar](/dev/mcmanual/mc-dev/assets/img/custom_command6.c278a977.png)
...
![avatar](/dev/mcmanual/mc-dev/assets/img/custom_command5.d41dcfca.png)
