...
目前给开发者提供了两种内嵌成就类型分别为[击杀生物事件](<#%E5%87%BB%E6%9D%80>)和[获得物品事件](<#%E8%8E%B7%E5%BE%97>)，开发者也可通过自己监听事件来判断玩家是否达成成就。

该系统还提供了自动调用的奖励机制，开发者只需在serverSystem编写奖励函数，在json中配置对应的命名空间与系统名，自定义成就系统即可在玩家完成成就时，自动调用对应的函数。（详见示例[CustomAchievementMod](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/13-模组SDK编程/60-Demo示例.html#CustomAchievementMod>)）

![POPO-screenshot-20220613-145720](/dev/mcmanual/mc-dev/assets/img/POPO-screenshot-20220613-145720.c15bf8ba.png)
...
title | 必选，成就节点标题，根节点的标题会在对应窗口显示时作为该窗口的主标题，该标题在点击节点时也会展示，也会用于完成成就时的弹框展示，[支持颜色代码 (opens new window)](<https://zh.minecraft.wiki/w/%E6%A0%B7%E5%BC%8F%E4%BB%A3%E7%A0%81>)  
...
node_description | 必选，用于描述该节点的具体信息，建议在这里写上成就目标与成就奖励，[支持颜色代码 (opens new window)](<https://zh.minecraft.wiki/w/%E6%A0%B7%E5%BC%8F%E4%BB%A3%E7%A0%81>)  
...
auto_sent_cloud | 可选，是否自动上传云成就，默认为True，当“is_cloud”也为True时才有作用，如果设置为自动上传会在每次调用[AddNodeProgress](<./../../../mcdocs/1-ModAPI/接口/自定义UI/自定义成就系统.html#addnodeprogress>)修改成就进度的时候，将属于云成就的节点进度上传。如果为False，由开发者自行选择上传时机。可以用[AddNodeProgress](<./../../../mcdocs/1-ModAPI/接口/自定义UI/自定义成就系统.html#addnodeprogress>)接口更新本地数据，并用一个变量存储相加每次的Delta，使用云成就上报接口[LobbySetAchievementStorage](<./../../../mcdocs/1-ModAPI/接口/成就.html#lobbysetachievementstorage>)，**仅联机大厅与Apollo有用**  
  
![POPO-screenshot-20220613-174917](/dev/mcmanual/mc-dev/assets/img/POPO-screenshot-20220613-174917.49850b0d.png)
...
title | 必选，成就节点标题，该标题在点击节点时会展示，也会用于完成成就时的弹框展示，[支持颜色代码 (opens new window)](<https://zh.minecraft.wiki/w/%E6%A0%B7%E5%BC%8F%E4%BB%A3%E7%A0%81>)  
unfinished_icon | 必选，成就未达成前的图标，路径为resource文件下的“texture/../..”  
finished_icon | 必选，成就达成后的图标，路径为resource文件下的“texture/../..”  
use_default_icon_background | 可选，是否使用提供的图标背景，默认为True，如果开发者想使用自己的背景图标，可设置为False，然后在上述的unfinished_icon/finished_icon中设置自己想要的图标样式，即一张图标中包括Icon与Icon的背景  
goal_details | 可选，如果是想使用提供的“击杀事件”或“获得事件”，需要按照要求进行内容填写，详参*[3.两个基础预设的成就类型](<#%E4%B8%A4%E4%B8%AA%E5%9F%BA%E7%A1%80%E9%A2%84%E8%AE%BE>)*  
goal_number | 必选，成就的目标值，例如“击杀3头牛”则为3，“获得5个苹果”则为5，如果是特殊事件如”进入下界“则玩家可自行设置为1  
node_description | 必选，用于描述该节点的具体信息，建议在这里写上成就目标与成就奖励，[支持颜色代码 (opens new window)](<https://zh.minecraft.wiki/w/%E6%A0%B7%E5%BC%8F%E4%BB%A3%E7%A0%81>)  
reward | 可选，注册可调用的奖励函数，“function_name”：函数名，“name_space”：nameSpace：systemName（即Mod注册RegisterSystem时填写的nameSpace和systemName，这里为serverSystem，不可为clientSystem），data为调用这个函数时传入的参数dict，该dict参数还会添加一个“playID”用于传输触发奖励的玩家ID  
...
![POPO-screenshot-20220613-150745](/dev/mcmanual/mc-dev/assets/img/POPO-screenshot-20220613-150745.d72c86f9.png)
...
![POPO-screenshot-20220613-162148](/dev/mcmanual/mc-dev/assets/img/POPO-screenshot-20220613-162148.a0b8f60f.png)

## 6.成就接口与事件

### 6.1接口

  * [GetNodeDetailInfo](<./../../../mcdocs/1-ModAPI/接口/自定义UI/自定义成就系统.html#getnodedetailinfo>)：获取对应玩家的对应节点信息
  * [SetNodeFinish](<./../../../mcdocs/1-ModAPI/接口/自定义UI/自定义成就系统.html#setnodefinish>)：设置对应玩家的对应成就节点完成
  * [AddNodeProgress](<./../../../mcdocs/1-ModAPI/接口/自定义UI/自定义成就系统.html#addnodeprogress>)：增加对应玩家的对应成就节点的成就进度
  * [GetChildrenNode](<./../../../mcdocs/1-ModAPI/接口/自定义UI/自定义成就系统.html#getchildrennode>)：获得该成就节点的下一级所有孩子节点的list


### 6.2成就事件

  * [AchievementCompleteEvent](<./../../../mcdocs/1-ModAPI/事件/世界.html#achievementcompleteevent>)：玩家完成自定义成就时触发该事件