...
方块实体的概念详见[官方wiki (opens new window)](<https://zh.minecraft.wiki/w/%E6%96%B9%E5%9D%97%E5%AE%9E%E4%BD%93>)
...
为false时，该方块不可被粘性活塞拉回  
  ...
  * **对于已有方块实体的方块，如[自定义刷怪箱](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/3-特殊方块/1-自定义刷怪箱.html>)，将无法再添加自定义方块实体。**
...
## 相关组件与事件
  ...
  * [ GetBlockEntityData接口 ](<../../../../mcdocs/1-ModAPI/接口/方块/方块实体.html#getblockentitydata>)

可用于管理方块实体内的数据。

  * [ ServerPlaceBlockEntityEvent事件 ](<../../../../mcdocs/1-ModAPI/事件/方块.html#serverplaceblockentityevent>)

当玩家手动放置含自定义方块实体的自定义方块时触发，此时可向该方块实体中存储数据。

  * [ ChunkGeneratedServerEvent事件 ](<../../../../mcdocs/1-ModAPI/事件/世界.html#chunkgeneratedserverevent>)

通过自定义特征放置含自定义方块实体的自定义方块时，在区块生成完毕时触发，其中包含了该区块中自定义方块实体信息列表，此时可向该方块实体中存储数据。

  * [ ServerBlockEntityTickEvent事件 ](<../../../../mcdocs/1-ModAPI/事件/方块.html#serverblockentitytickevent>)
...
[CustomBlocksMod](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/13-模组SDK编程/60-Demo示例.html#CustomBlocksMod>)中，`customblocks:customblocks_test_block_entity`方块配置了自定义方块实体，具体配置如下：

即该方块在距离玩家不远时会每秒发送20次ServerBlockEntityTickEvent事件，且无法被粘性活塞拉回。


![block_entity_1](/dev/mcmanual/mc-dev/assets/img/block-entity-1.36542f24.png)

  * 方块放置

customBlocksServer.py中注册了对ServerPlaceBlockEntityEvent事件的监听，在回调里通过blockEntityData组件向每个由玩家手动创建的自定义方块实体中写入数据：
[code] def ServerPlaceBlockEntityEvent(self, args):
...
[code] def ServerBlockUseEvent(self, args):
...
```
  * 方块实体tick
[code] def OnBlockEntityTick(self, args):
...
[code] def ServerPlayerTryDestroyBlockEvent(self, args):
