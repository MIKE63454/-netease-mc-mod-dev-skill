...
该功能类似自定义箱子，使用原生c++驱动背包界面，实现自定义容器界面的方块。 包含长按分堆、双击合堆、右键拖放等功能，方块破坏后内部物品会掉落，方便开发者制作自定义熔炉等界面，而无需考虑复杂的UI逻辑，数据同步逻辑等。
...
## hopper字段说明
  ...
  * output_slot：类型为list。当"output": true时代表output_slot字段生效，"output_slot": [1,10] 代表 1和10 支持漏出
