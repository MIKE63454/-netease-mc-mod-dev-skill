...
![常见跳转演示](/dev/mcmanual/mc-dev/assets/img/常见跳转演示.105ceaef.gif) 从目录一直跳转到章节内的某一页
...
![页组的概念](/dev/mcmanual/mc-dev/assets/img/页组的概念.763d86e4.png)

如上图所示的例子，这本书有8页（页号从0开始算），由3个PageGroup管理，当你需要请求页面4，就只需要向PageGroup3请求它的第一个页面即可。  
请求的地址格式为 **PageGroup3的名称 + "/0**"， 0表示第0页。

尽管有了分组管理，但是仍然不像书本拥有目录，章节等层级结构，PageGroup仍然是相同层级的，因此有了 **Book** ，**Category** ，**Entry** 这三个对象，它们均继承自**PageGroup** ，除了指向页面外还能指向其子页组，具体可见示例[CustomBookMod](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/13-模组SDK编程/60-Demo示例.html#CustomBookMod>)中的“**behavior_pack/customBooks/Book1** ”，其对应的物品中文名称为”**链接测试书本“** 。

![层级跳转演示](/dev/mcmanual/mc-dev/assets/img/层级跳转演示.c8d77ed6.gif) ![层级概念](/dev/mcmanual/mc-dev/assets/img/层级概念.a95e7216.png)
...
自定义书系统会按照层级关系设置这三个页组的路径值如下：
    ...
    * **Book1** 的名称为其[identifier](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/5-自定义书本/01-自定义基础书本.html#书本identifier>)，其**绝对路径** 为“**/Book1的名称** "，即“**/Book1** "。
    * **Category1** 的名称为其[identifier](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/5-自定义书本/01-自定义基础书本.html#目录identifier>)，其**绝对路径** 为“**Category1的父页组的绝对路径/Category1的名称** "，即“**/Book1/Category1** "。
    * **Entry1** 的名称为其[identifier](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/5-自定义书本/01-自定义基础书本.html#章节identifier>)，其**绝对路径** 为“**Entry1的父页组的绝对路径/Entry1的名称** "，即“**/Book1/Category1/Entry1** "。
...
属性名 | 描述 | 枚举值  
---|---|---  
TextSize | 文字字体大小（整型） | infotext：用于组件的标注文本字体大小，值为6px  
footprint：页面脚注的文本字体大小，值为8px  
content：内容文本字体大小，值为10px  
smallTitle：小标题，值为12px  
middleTitle：中标题，值为14px  
title：大标题，值为16px  
Colors | 颜色（三维浮点数） | TextDefault：默认字体颜色，值为(48, 3, 3, 255)归一化后的取值  
BookTitle：书本首页标题字体颜色，值为(249, 221, 166, 255)归一化后的取值  
SubTitle：页面标题的字体颜色，值为(72.0, 30, 23, 255)归一化后的取值  
  
TextAlign | 文本对齐方式 | Left：文本框大小固定，文本左对齐  
Center：文本框大小固定，文本居中对齐  
Right：文本框大小固定，文本右对齐  
Fit_Left：文本框大小自适应，文本左对齐  
Fit_Center：文本框大小自适应，文本居中对齐  
Fit_Right：文本框大小自适应，文本右对齐  
ImageReszieRule | 图片缩放方式 | Ninesliced：九宫格方式缩放，左右上下边距为0px  
Fit：图片比例固定，图片会尽量占满给定空间  
Images | 预设图片路径 | blank：完全透明的空白图片  
categoryDefaultIcon： 目录默认图标  
![](data:image/png;base64,)  
lockBtn_dark：目录默认的锁图标  
![](data:image/png;base64,)  
sqrtPanel_light：实体显示的默认背景图  
![](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAIAgMAAAC5YVYYAAAABGdBTUEAALGPC/xhBQAAACBjSFJNAAB6JgAAgIQAAPoAAACA6AAAdTAAAOpgAAA6mAAAF3CculE8AAAADFBMVEWCbUi8o3j24b4AAADR/zrqAAAAAWJLR0QDEQxM8gAAAAd0SU1FB+ULFgsLOqFS9lsAAAARSURBVAjXY2BgYBANQ0daqwAeawNXRpElLQAAAABJRU5ErkJggg==)  
progressBar_light：进度条默认前景图  
![](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAGUAAAAIAgMAAACdeNnYAAAABGdBTUEAALGPC/xhBQAAACBjSFJNAAB6JgAAgIQAAPoAAACA6AAAdTAAAOpgAAA6mAAAF3CculE8AAAACVBMVEWPSDNlKBYAAACFhrKmAAAAAWJLR0QCZgt8ZAAAAAd0SU1FB+ULFgoeL/r4npMAAAASSURBVAjXY2CgIwjFBRyoLgUAPFMiNfErB9kAAAAASUVORK5CYII=)  
progressBar_dark：进度条默认背景图  
![](data:image/png;base64,)  
...
该对象管理**所有的书本对象** ，它会保存所有书本的[浏览历史记录](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/5-自定义书本/01-自定义基础书本.html#浏览历史记录>)以及[链接地址](<#%E7%BB%9D%E5%AF%B9%E8%B7%AF%E5%BE%84>)，提供所有书本的公共方法，通过该对象能获取到当前正在打开的书本对象，以及调用全局消息显示，注册自定义页面，跳转书本页面。
...
数据类型 | 说明  
---|---  
[BookConfig](<#%E4%B9%A6%E6%9C%AC%E9%85%8D%E7%BD%AE>) | 书本配置常量  
  * 示例
...
参数名 | 数据类型 | 说明  
---|---|---  
bookName | str | 书本的名称，也就是书本的[identifier](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/5-自定义书本/01-自定义基础书本.html#书本identifier>)  
  * 返回值
...
参数名 | 数据类型 | 说明  
---|---|---  
position | tuple(int, int) | 消息显示的位置（[全局坐标系](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/5-自定义书本/01-自定义基础书本.html#页面编写>)）  
msg | str | 消息的文本内容  
默认值为“**点击无任何说明，请填充数据** "  
  * 返回值
...
可见[CustomBookMod](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/13-模组SDK编程/60-Demo示例.html#CustomBookMod>)中的**behavior_pack/tutorialScripts/pages/buttonPage.py** 这一自定义页面的这块代码，这里实现的功能就是点击按钮后在按钮的中心位置显示文本信息。
[code] def ShowMsg(self, msg):
            # 获取所有书本得管理对象
            bookManager = clientApi.GetBookManager()
...
可见[CustomBookMod](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/13-模组SDK编程/60-Demo示例.html#CustomBookMod>)中的**behavior_pack/tutorialScripts/tutorialClientSystem.py** 这块代码，这里编写了几个自定义的页面类，并为它们注册页的名称，这个名称将会在编写书本json的页面时使用，详细可见[json使用页面](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/5-自定义书本/01-自定义基础书本.html#页面编写>)。

**TutorialClientSystem.py**
[code] # MyNoTitlePage是自定义的一个页面类
...
参数名 | 数据类型 | 说明  
---|---|---  
addr | str | 书本页面地址，自定义书本的页面跳转机制和浏览器的地址导航是类似的，详见[页面跳转机制](<#%E9%A1%B5%E9%9D%A2%E8%B7%B3%E8%BD%AC%E6%9C%BA%E5%88%B6>)  
  * 返回值
...
可见[CustomBookMod](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/13-模组SDK编程/60-Demo示例.html#CustomBookMod>)中的**behavior_pack/tutorialScripts/pages/addrPage.py** 这一自定义页面的这块代码，这里实现的功能就是点击按钮后跳转到指定章节的指定页面。
[code] def GoToPage(self, addr):
            # 获取所有书本得管理对象
            bookManager = clientApi.GetBookManager()
            # 跳转到地址
            bookManager.To(addr)
...
备注
  ...
  * 只有发生跳转（调用该函数的时候），才可以点击”**返回“**按钮进行返回（和浏览器的后退是类似的功能），常见的从书本首页到目录首页，从目录首页到章节，这些操作都属于跳转，所以返回的时候就会回到跳转前的状态，而** ”上一页“按钮**，**”下一页“按钮** 并不是跳转操作，具体可以参见[页面跳转机制](<#%E9%A1%B5%E9%9D%A2%E8%B7%B3%E8%BD%AC%E6%9C%BA%E5%88%B6>)。
...
因为用法是一样的，这里仅举一个例子，更具体的用法可参见[CustomBookMod](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/13-模组SDK编程/60-Demo示例.html#CustomBookMod>)的自定义页面部分
[code] import mod.client.extraClientApi as clientApi
...
例子可参见[获取组件类](<#%E8%8E%B7%E5%8F%96%E7%BB%84%E4%BB%B6%E7%B1%BB>)
...
参数名 | 数据类型 | 说明  
---|---|---  
[identifier](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/5-自定义书本/01-自定义基础书本.html#目录identifier>) | str | 目录的唯一标识。  
  * 返回值
...
参数名 | 数据类型 | 说明  
---|---|---  
[identifier](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/5-自定义书本/01-自定义基础书本.html#章节identifier>) | str | 章节的唯一标识。  
  * 返回值
...
该对象是管理一组页面的，详细见[对象分析](<#%E5%AF%B9%E8%B1%A1%E5%88%86%E6%9E%90>)。一般开发者不会直接使用该类，而是使用该类的三个继承类：**Book，Category，Entry** ，所以这里的API实际上是这三个类的公共API。

### GetAddr

  * 描述

获取该页组的[绝对路径](<#%E7%BB%9D%E5%AF%B9%E8%B7%AF%E5%BE%84>)
...
该对象继承**PageGroup** ，是包含书本首页的页组，表示一个书本的对象，详细见[对象分析](<#%E5%AF%B9%E8%B1%A1%E5%88%86%E6%9E%90>)。
...
float | 当前子页组解锁的进度，范围为[**0, 1**]，**0** 表示**0%** ，**1** 表示**100%**  
  * 示例
[code] # 比如获取书本当前的解锁进度，如果书本下有两个已经解锁的一级子目录，那么其解锁进度为100%，如果一个解锁一个未解锁，则为50%
        import mod.client.extraClientApi as clientApi
        bookInstance = clientApi.GetBookManager().GetBookInstance("customBook")	# 获取书本名称为"customBook"的书本管理对象
        if bookInstance:
            bookObj = bookInstance.GetBook()
...
该对象继承**PageGroup** ，是包含目录首页的页组，表示一个目录的对象，详细见[对象分析](<#%E5%AF%B9%E8%B1%A1%E5%88%86%E6%9E%90>)。

### GetSons

  * 描述

获取子页组（其子页组可以是Category对象，也可以是Entry对象，因为目录是允许嵌套的，可参见[目录嵌套](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/5-自定义书本/01-自定义基础书本.html#目录嵌套>)。）
...
获取其父页组（其父页组可以是Category对象，也可以是Book对象，因为目录是允许嵌套的，可参见[目录嵌套](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/5-自定义书本/01-自定义基础书本.html#目录嵌套>)。）
...
该对象继承**PageGroup** 的页组，表示一个章节的对象，详细见[对象分析](<#%E5%AF%B9%E8%B1%A1%E5%88%86%E6%9E%90>)。
