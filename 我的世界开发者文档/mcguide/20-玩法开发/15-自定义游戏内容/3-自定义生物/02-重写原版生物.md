...
请结合示例[CustomEntityMod](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/13-模组SDK编程/60-Demo示例.html#CustomEntityMod>)来阅读该文档。

## 1\. 重写生物行为

如示例[CustomEntityMod](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/13-模组SDK编程/60-Demo示例.html#CustomEntityMod>)中，在behavior_pack/entities下添加custom_pig.json，identifier依然为minecraft:pig，然后加上了**苦力怕** 的攻击组件minecraft:nearest_attackable_target和minecraft:target_nearby_sensor，以及爆炸事件，使得它会攻击玩家并且爆炸消失。
...
与重写生物行为一样，我们可以通过重写资源包下的entity定义、贴图、动画、模型、渲染控制器等json文件来重写生物的外观，如示例[CustomEntityMod](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/13-模组SDK编程/60-Demo示例.html#CustomEntityMod>)中，猪穿上了屠夫的贴图。
[code] {
