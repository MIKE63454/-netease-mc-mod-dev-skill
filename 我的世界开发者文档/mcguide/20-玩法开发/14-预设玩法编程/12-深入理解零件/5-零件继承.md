...
实现：编辑器内置已有触发器零件，能够监听实体进入/停留/离开某个区域，只需要在这之上补充部分逻辑即可
  ...
  2. 需要新建一个空零件（样例的零件名为SpecialArea） ![z_4](/dev/mcmanual/mc-dev/assets/img/z_4.6a22695d.png)

  3. 将零件的脚本与零件对应的属性脚本都继承对应的触发器脚本部分 ![z_2](/dev/mcmanual/mc-dev/assets/img/z_2.d7034f64.png)![z_3](/dev/mcmanual/mc-dev/assets/img/z_3.6c85ea8f.png)

  4. 编写逻辑代码, 链接[在此 (opens new window)](<https://g79.gdl.netease.com/PartsInheritSample_SpecialArea.7z>)，接口可在官方的api文档页面中找到[api](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/14-预设玩法编程/13-PresetAPI/预设对象/通用/SDK接口封装SdkInterface.html>)
```python
    # -*- coding: utf-8 -*-
...
```
  5. 修改部分属性参数确保效果正确
     ...
     4. 因为我们只需要服务端的监听，就修改属性只监听服务端 ![z_5](/dev/mcmanual/mc-dev/assets/img/z_5.234c0909.png)
  6. 运行测试

可一看到提示，僵尸怪物都有buff，女巫没有，人物右侧有debuff，可以自行运行感受 ![z_1](/dev/mcmanual/mc-dev/assets/img/z_1.b396db70.png)


## 注意事项

  1. 继承零件不仅可以继承内置零件，也可以继承自己写的零件，不过由于某些原因，继承自己写的零件时路径需要注意 ![z_6](/dev/mcmanual/mc-dev/assets/img/z_6.add970c6.png)
