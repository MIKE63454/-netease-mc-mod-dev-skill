...
### 使用feature_rules控制建筑生成

参考教程：[自定义特征](</dev/mcmanual/mc-dev/mcguide/20-玩法开发/15-自定义游戏内容/4-自定义维度/4-自定义特征.html>)
...
**缺点：**
  ...
  * feature_rules文件中的molang表达式不宜过于复杂
  * 复杂的表达式可能导致内存占用增加，影响游戏加载速度


### 通过API放置structure

参考API：[PlaceStructure](<../../../mcdocs/1-ModAPI/接口/世界/地图.html#placestructure>)
...
方块调色板可用于保存建筑数据。使用[GetBlockPaletteBetweenPos](<../../../mcdocs/1-ModAPI/接口/世界/方块组合.html#getblockpalettebetweenpos>)等接口获取方块调色板后，使用[SerializeBlockPalette](<../../../mcdocs/1-ModAPI/接口/方块/方块调色板.html#serializeblockpalette>)将调色板转化成字典后，手动存入配置文件中。

**优点：**

  * 方块调色板是高度压缩的字典结构，可最大程度节约内存
  * 对应的放置接口[SetBlockByBlockPalette](<../../../mcdocs/1-ModAPI/接口/世界/方块组合.html#setblockbyblockpalette>)是原生c++批量放置，性能较好
  * 可实现复杂的生成规则
...
## 注意事项
  ...
  * 如果想进一步优化，可以将配置存储为json文件，通过json方式使用和卸载，相关接口[GetModConfigJson](<../../../mcdocs/1-ModAPI/接口/通用/工具.html#getmodconfigjson>)。
  * 可以使用`zlib`库压缩数据。
...
优化前的内存占用（使用方块探针工具）： ![image](/dev/mcmanual/mc-dev/assets/img/before_optimization.a79effe8.png)

优化后的内存占用（使用方块探针工具）： ![image](/dev/mcmanual/mc-dev/assets/img/after_optimization.d5e5b326.png)
